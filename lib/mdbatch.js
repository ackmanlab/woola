...
function markdownBatch(
  ...
  fs.mkdir(dstPath, (err, undefined) => {
      if (err && !overwrite) {
        console.log(`Error markdownBatch: ${err.message}. No files processed since overwrite: ${overwrite}`);
      } else if (err && overwrite) {
        console.log(`Warning markdownBatch: ${err.message}. Overwriting files since overwrite: ${overwrite}`);
