...
// .then(datain => { try {
//   return options.handleFile(datain,file,dstPath)
//   } catch (err) {
//      err;
//     }
//   })
...
const processFiles = (options) => {
  ...
  .then(files => { return buildDataArray(files,options) })
  .then(resultsArray => { return options.handleStack(resultsArray) })
  .then(resultsArray => {
    resultsArray.forEach(f => {
      // console.log( f.file + ': ' + f.url)
      writeFile(path.join(options.dstPath,f.file), f.content)
    })
  })
  .catch (err => { console.log(err) })
}

function batchfiles (options) {
  //app start
  fs.mkdir(options.dstPath, (err, undefined) => {
      if (err && !options.overwrite) {
        console.log(`Error batchfiles: ${err.message}. No files processed since overwrite: ${options.overwrite}`)
      } else if (err && options.overwrite) {
        console.log(`Warning batchfiles: ${err.message}. Overwriting files since overwrite: ${options.overwrite}`)
