// Modify or make new template objects below 

//need helper function for linking to site vs page specific css (for including different types depending on layout, category etc)
//<link rel="stylesheet" href="${site.baseurl}/css/main.css">

//need to edit helper function for page link layout in header

// ---template build masters---
const templates = {
  build: {
...
templates.layouts = {
  ...
  page: (site, helpers, idx) => {
    const data = site.pages[idx]
    return (
`<article class="post">
...
`<article class="post" itemscope itemtype="http://schema.org/BlogPosting">
  ...
  home: (site, helpers, idx) => {
    const data = site.pages[idx]
    let excludes = []
    const intersect = site.pages[idx].categories.filter(x => { return (site.options.filterCategories.indexOf(x) !== -1) })
    if (!intersect[0]) {
      site.options.filterCategories.forEach(x => {
        if (!(intersect.find(n => x == n))) { excludes.push(x) }
      })
    } else {
...
`<div class="home">
  ...
  localnav: (site, helpers, idx) => {
    // TODO: implement modular category nav index creation
    const data = site.pages[idx]
    return (
`<div class="home">
  <!-- <h1 class="page-heading">Posts</h1> -->
  ...
  <h2>
  ${helpers.pushBananaSlugIcon(site)}
  </h2>
  </li>
...
pushPageLinks: function (site) {
  ...
  site.pages.forEach(p => { 
    if (p.layout === 'page') {
        ans.push(`<a class="page-link" href="${site.baseurl + p.url}">${p.title}</a>`)
      }}) 
  return (ans.join('\n'))
},
...
pushPostLinks: function (site, excludes=['local','pri']) {
  ...
  site.pages.forEach(p => { 
    ...
    if (p.layout === 'post' && !intersect[0]) {
        ans.push(
`<li>
<span class="post-meta"><time datetime="${new Date(p.date).toISOString()}" itemprop="datePublished">${new Date(p.date).toDateString().slice(4).replace(/ ([0-9]{4})/, ', $1')}</time></span>
<h2>
<a class="post-link" href="${site.baseurl + p.url}">${p.title}</a>
</h2>
</li>`)
      }})
  return (ans.join('\n'))
...
pushPageTitle: function (site, data) {
...
pushBananaSlugIcon: function (site, icons) {
  // return social link and icon
  if (site.local_url) {
    return (
      `<a class="post-link" href="${site.local_url}"><span class="coloricon">${this.icons.bananaslug}</span><span>Local</span></a>`
    )
  } else {
    return ''
  }
