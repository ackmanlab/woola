...
let connectApp
let httpPort
const site = require('./appconfig.js')(flags,msg,errormsg)
...
const linkify = (page) => new Promise((resolve, reject) => {  
  jsdom.env(page.content, (err, window) => {
    ...
    for (let i = 0; i < l; i++) {
      ...
      markdownFile = href.replace(path.join('file://', __dirname), site.baseDir) + '.md'
...
const printPDF = (fileName, res, query) => buildHTMLFromMarkDown(fileName, query)
  .then(html => {
    ...
    try {
      writepdf(html, outFile, site.pdfconfig[site.pdfconfig.use])
    } catch (err) {
