...
flags.version(pkg.version)
  ...
  .option('-a, --address [type]', 'Serve on ip/address [address]', '127.0.0.1') //localhost or 127.0.0.1
...
const startLiveReloadServer = (HTTP_SERVER) => new Promise(resolve => {
...
// });
  ...
  const handleChanges = (changedFile, changeType) => {
    ...
    if (changed) {
      ...
      //   return buildHTMLFromMarkDown(changedFile,null)
      // });

      // io.sockets.emit('change', console.log('hello there'));
