...
<head>
  ...
  <link rel="stylesheet" href="http://localhost:${httpPort}/css/katex.min.css">
 
 <script src="http://localhost:${httpPort}/css/highlight.pack.js"></script>
  <link rel="stylesheet" href="http://localhost:${httpPort}/css/github-gist.css">
...
const printPDF = (fileName, res, query) => buildHTMLFromMarkDown(fileName, query)
  .then(html => {
    // res.writeHead(200)
    res.end(html)
    const localurl = 'http://localhost:' + httpPort + '/' + path.parse(fileName).base + '?pdf'
...
const httpRequestHandler = (req, res) => {
  ...
  try {
    ...
    if (query === 'pdf') {
      ...
      const localurl = 'http://localhost:' + httpPort + '/' + path.parse(fileName).base + '?print'
      const outFile = path.join(path.parse(fileName).dir, path.parse(fileName).name + '.pdf')
      writepdf(localurl, outFile, site.pdfconfig[site.pdfconfig.use])
