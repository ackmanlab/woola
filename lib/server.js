...
const opts = require('../config.js').options
// site.time = new Date()
...
// Path Variables
// const lessStyle = path.join(__dirname, 'less/bootstrap-custom.less')
// const lessStyle = path.join(process.cwd(), 'css/bootstrap-custom.less')
...
flags.version(pkg.version)
  ...
  .option('-s, --less [type]', 'Path to Less styles [less]', '../css/bootstrap-custom.js')
...
const dir = flags.dir
const lessStyleData = require(flags.less)
...
try {
  var pdfConfig = require('../css/pdf-config-cover.json')
...
const markdownToHTML = (data, fileName) => new Promise(resolve => {
  resolve(mdparse(data, fileName, opts))
})
...
const printPDF = (fileName, res, query) => buildHTMLFromMarkDown(fileName, query)
  .then(html => {
    ...
    pdf.create(html, pdfConfig).toFile( outFile + '.pdf', (err, res) => {
...
const compileAndSendDirectoryListing = (fileName, res) => {
  ...
  buildStyleSheet(lessStyleData).then( css => {
    const html = (
