  ...
  <script>
    function elementSelect(id) {
      ...
      el.all = function(selector) {
        return elementSelect(el.querySelectorAll(selector))
      }

      el.each = function(fn) {
        for (var i = 0, len = el.length; i < len; ++i) {
          fn(elementSelect(el[i]), i)
        }
      }

      el.getClasses = function() {
        return ( (this.getAttribute('class')) ? this.getAttribute('class').split(/\s+/) : [null] )
      }
    ...
    function search() {
      var str = elementSelect('search').value.toLowerCase()
      var links = elementSelect('files').all('a')
      ...
      links.each(link => {
        ...
        if (str.length && ~text.indexOf(str)) {
          link.addClass('highlight-search')
        } else {
          link.removeClass('highlight-search')
        }
      })
    }

    elementSelect(window).on('content loaded', () => {
      elementSelect('search').on('keyup', search);
    });
  </script>

</head>
<body>
<input id="search" type="type" placeholder="" autocomplete="off" />
  <article class="markdown-body">
