#!/usr/bin/env node
// mdwatcher: A fork of server.js from <a href="https://www.npmjs.com/package/markserv">MarkServ 0.1.3 by Alistair MacDonald <al@pwn.io> MIT License
// 2017-08-01 00:39:35 James Ackman

const markdownExtensions = [ '.markdown', '.md', '.mmd', '.txt', '.text' ]
const watchExtensions = [ '**/*.markdown', '**/*.md', '**/*.mmd', '**/*.txt', '**/*.text', '**/*.html' ]
...
try {
  var siteConfig = require(path.resolve(process.cwd(),'./config.js'))
  siteConfig.time = new Date()
} catch (err) {
errormsg('error')
  .write('./config.js not found', err)
  .reset().write('\n')
}
...
const markdownToHTML = (data, fileName) => new Promise(resolve => {
  resolve(mdparse(data, fileName, siteConfig.options))
})
